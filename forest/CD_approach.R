...
EFA.Comp.Data <- function(Data, F.Max, N.Pop = 10000, N.Samples = 500, Alpha = .30, Graph = F, Spearman = F, use)
{
  ...
  cor.Data <- EGAnet::polychoric.matrix(veri_kategorik[[i]]) #cor(Data,use=use)#
  ...
  while ((F.CD <= F.Max) & (Sig))
  {
    ...
    for (j in 1:N.Samples)
    {
      ...
      RMSR.Eigs[j,F.CD] <- sqrt(sum((Eigs.Samp - Eigs.Data) * (Eigs.Samp - Eigs.Data)) / k) #SRMR yerine RMSE'de kullanilabilir
    }
...
Factor.Analysis <- function(Data, Corr.Matrix = FALSE, Max.Iter = 50, N.Factors = 0, Cor.Type, use)
{
  ...
  while ((Change >= Criterion) & (Iter < Max.Iter)) # >= --> > yapildi, duzelti 13.12.2023 15.24  1000.5k.madde16.2f.cor00.ofy60.sol kosulunda
  {
    Iter <- Iter + 1
